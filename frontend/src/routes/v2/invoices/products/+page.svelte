...
<PageHeader title="Products">
  ...
  {#snippet actions()}
    {#if canManage}
      <a class="v2-btn v2-btn-primary" href="/v2/invoices/products/new"><Plus />New product</a>
    {/if}
  {/snippet}
</PageHeader>
  ...
  <div class="v2-pad" style="padding-top:18px;padding-bottom:32px">
    {#if data.products.length === 0}
      ...
      >
        ...
        {#snippet actions()}
          {#if canManage}
            <a class="v2-btn v2-btn-primary" href="/v2/invoices/products/new">New product</a>
          {:else}
        ...
        <div class="v2-card" style="overflow:hidden;margin-bottom:20px">
          {#each group.items as p (p.id)}
            <div class="v2-setting" style={p.is_active ? '' : 'opacity:.62'}>
              ...
              {#if canManage}
                <a class="prod-edit" href="/v2/invoices/products/{p.id}/edit" title="Edit {p.name}">
                  <Pencil size={14} />
                </a>
              {/if}
            </div>
