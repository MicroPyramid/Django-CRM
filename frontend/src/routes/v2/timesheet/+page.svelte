<script>
  ...
  function shiftWeek(/** @type {number} */ deltaDays) {
    ...
    goto(`/v2/timesheet?${qs.toString()}`, { keepFocus: true, noScroll: true });
  }

  /** Back to the current ISO week (no params → server default). */
  function thisWeek() {
    goto('/v2/timesheet', { keepFocus: true, noScroll: true });
  }
          ...
          <div class="v2-next-text">
            {#each week.days as d (d.date)}
              {#each d.entries.filter((e) => e.is_running) as e (e.id)}
                ...
                <a href="/v2/tickets/{e.case.id}" style="color:inherit">{e.case.name}</a>
              {/each}
            {/each}
          </div>
        </div>
