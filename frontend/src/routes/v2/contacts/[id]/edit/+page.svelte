...
<PageHeader title="Edit {contact.name}" center>
  {#snippet crumb()}
    ...
    <a href="/v2/contacts/{contact.id}">{contact.name}</a>
  {/snippet}
  {#snippet sub()}
    {[
      contact.title,
  ...
  <form class="v2-form" method="POST" action="?/save" use:enhance={check} novalidate>
    {#if saved}
      <div class="v2-next" style="margin-bottom:18px" role="status">
        ...
        <a class="v2-btn" href="/v2/contacts/{contact.id}">Back to the contact</a>
      </div>
    {/if}
    ...
    <div class="actions">
      ...
      <a class="v2-btn" href="/v2/contacts/{contact.id}">Cancel</a>
    </div>
  </form>
</div>
