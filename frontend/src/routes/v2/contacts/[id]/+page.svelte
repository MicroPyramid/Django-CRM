...
<PageHeader title={contact.name} record>
  ...
  {/snippet}
  {#snippet crumb()}
    <a href="/v2/contacts">Contacts</a>
    <ChevronRight size={12} />
    {#if contact.account}
      <a href="/v2/accounts/{contact.account.id}">{contact.account.name}</a>
  ...
  {#snippet actions()}
    ...
    {/if}
    <a class="v2-btn v2-btn-primary" href="/v2/contacts/{contact.id}/edit"><Pencil />Edit</a>
  {/snippet}
</PageHeader>
...
<div style="display:flex;flex:1;min-height:0;overflow:hidden">
        ...
        {#if headline}
          <div style="margin-bottom:20px">
            <NextAction
              ...
              href="/v2/contacts/{contact.id}/edit"
  ...
  <aside class="v2-rail">
    ...
    <dl class="v2-kv">
      ...
      <dd>
        {#if contact.account}
          <a href="/v2/accounts/{contact.account.id}" style="color:inherit"
      ...
      {#if contact.other_accounts.length}
        ...
        <dd>
          {#each contact.other_accounts as other, i (other.id)}
            {i > 0 ? ', ' : ''}<a href="/v2/accounts/{other.id}" style="color:inherit"
              >{other.name}</a
            >
          {/each}
        </dd>
