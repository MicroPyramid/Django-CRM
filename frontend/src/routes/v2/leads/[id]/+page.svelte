...
<PageHeader title="{lead.first_name} {lead.last_name}" record>
  ...
  {#snippet actions()}
    ...
    {/if}
    <a class="v2-btn" href="/v2/leads/{lead.id}/edit"><Pencil />Edit</a>
  {/snippet}
</PageHeader>
        ...
        {#if headline}
          <div style="margin-bottom:18px">
            <NextAction
              ...
              href={headline.action ? `/v2/leads/${lead.id}/edit` : null}
              tone={headline.tone}
            />
          </div>
        {/if}
          ...
          <div class="dup" role="note">
            ...
            <div style="font-size:12.5px;line-height:1.55">
              {#each duplicates as d, i (d.id)}
                {i > 0 ? ', ' : ''}<a href="/v2/leads/{d.id}">{d.name}</a> shares {d.matched_on}
