...
<PageHeader title="Edit ticket" center>
  {#snippet crumb()}
    ...
    <a href="/v2/tickets/{ticket.id}">{ticket.name}</a>
  {/snippet}
</PageHeader>
  ...
  <form class="v2-form" method="POST" action="?/save" use:enhance={check} novalidate>
    ...
    <p class="v2-sub" style="font-size:12px;margin:6px 0 0">
      {#if data.server.account}
        Linked to <a href="/v2/accounts/{data.server.account.id}">{data.server.account.name}</a>,
    ...
    <div class="actions">
      ...
      <a class="v2-btn" href="/v2/tickets/{ticket.id}">Cancel</a>
    </div>
  </form>
</div>
