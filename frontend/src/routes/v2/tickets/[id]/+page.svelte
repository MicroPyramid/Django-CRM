...
<PageHeader title={ticket.name} record>
  ...
  {/snippet}
  {#snippet crumb()}
    <a href="/v2/tickets">Tickets</a>
    {#if ticket.account}
      <ChevronRight size={12} />
      <a href="/v2/accounts/{ticket.account.id}">{ticket.account.name}</a>
    {/if}
  {/snippet}
  {#snippet actions()}
    <a class="v2-btn" href="/v2/tickets/{ticket.id}/edit"><Pencil size={12} />Edit</a>
    {#if ticket.is_open}
...
<div style="display:flex;flex:1;min-height:0;overflow:hidden">
  ...
  <aside class="v2-rail">
    ...
    {#if ticket.account}
      ...
      <a
        class="v2-rail-row"
        href="/v2/accounts/{ticket.account.id}"
    ...
    {#if alsoOpen.length}
      ...
      {#each alsoOpen as t (t.id)}
        <a class="v2-rail-row" href="/v2/tickets/{t.id}" style="color:inherit;text-decoration:none">
          <div>
