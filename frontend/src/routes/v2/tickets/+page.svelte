...
<PageHeader title="Tickets">
  ...
  {/snippet}
  {#snippet actions()}
    <a class="v2-btn v2-btn-primary" href="/v2/tickets/new"><Plus />New ticket</a>
  {/snippet}
</PageHeader>
...
<div class="v2-scroll">
  {#if tickets.length === 0}
    ...
    >
      ...
      {#snippet actions()}
        <a class="v2-btn v2-btn-primary" href="/v2/tickets/new">New ticket</a>
        {#if !data.showAll}
          <a class="v2-btn" href="/v2/tickets?all=1">Show closed too</a>
        {/if}
        <a class="v2-btn" href="/v2/solutions">Knowledge base</a>
      {/snippet}
    </EmptyState>
  {:else}
      ...
      <table class="v2-table">
        ...
        <tbody>
          {#each tickets as t (t.id)}
            ...
            <tr>
              <td data-m="title">
                <a class="v2-row-link" href="/v2/tickets/{t.id}">
              ...
              <td class="v2-muted" style="font-size:12.5px">
                {#if t.account}
                  <a class="v2-row-link" href="/v2/accounts/{t.account.id}">{t.account.name}</a>
                {:else}
                  No account
                {/if}
              </td>
    ...
    <p class="v2-sub v2-pad" style="font-size:12px;padding-bottom:24px">
      ...
      {#if !data.showAll}
        · <a href="/v2/tickets?all=1" style="color:inherit">include closed</a>
      {:else}
        · <a href="/v2/tickets" style="color:inherit">open only</a>
      {/if}
    </p>
  {/if}
</div>
