...
<PageHeader title="Edit {deal.name}" center>
  {#snippet crumb()}
    ...
    <a href="/v2/pipeline/{deal.id}">{deal.name}</a>
  {/snippet}
  {#snippet sub()}
  ...
  <form class="v2-form" method="POST" action="?/save" use:enhance={check} novalidate>
    {#if saved}
      <div class="v2-next" style="margin-bottom:18px" role="status">
        ...
        <a class="v2-btn" href="/v2/pipeline/{deal.id}">Back to the deal</a>
      </div>
    {/if}
      ...
      <div class="v2-field">
        ...
        {:else if amountIsCalculated}
          <p class="v2-hint" id="h-amount">
            <a href="/v2/pipeline/{deal.id}">{server.line_item_count} line items</a> add up to
    ...
    <div class="actions">
      ...
      <a class="v2-btn" href="/v2/pipeline/{deal.id}">Cancel</a>
