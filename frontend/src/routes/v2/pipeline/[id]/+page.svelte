...
<PageHeader title={deal.name} record>
  {#snippet crumb()}
    <a href="/v2/pipeline">Pipeline</a>
    <ChevronRight size={12} />
    <a href="/v2/accounts/{deal.account.id}">{deal.account.name}</a>
  {/snippet}
  {#snippet actions()}
    ...
    <a class="v2-btn" href="/v2/pipeline/{deal.id}/edit">Edit</a>
  {/snippet}
</PageHeader>
...
<div style="display:flex;flex:1;min-height:0;overflow:hidden">
  ...
  <aside class="v2-rail">
    ...
    {#each contacts as c (c.id)}
      <!-- A link now that `/v2/contacts/<uuid>` resolves. These names were
           plain text because the contacts module was still fixtures. -->
      <a class="v2-rail-row" href="/v2/contacts/{c.id}" style="color:inherit;text-decoration:none">
