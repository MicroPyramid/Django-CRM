...
<PageHeader title={account.name} record>
  ...
  {/snippet}
  {#snippet actions()}
    <a class="v2-btn" href="/v2/accounts/{account.id}/edit">Edit</a>
  {/snippet}
</PageHeader>
    ...
    {#if headline}
      <div style="margin-bottom:16px">
        <!--
          ...
          pointed at `/v2/invoices/<uuid>` while invoices is still fixtures
        ...
        <NextAction
          ...
          href={stalled.length ? `/v2/pipeline/${stalled[0].id}` : null}
          tone="rust"
        />
      </div>
    {/if}
    ...
    >
      ...
      <section class="v2-card" style="overflow:hidden">
        <div class="v2-card-head">
          ...
          <a href="/v2/pipeline">View all</a>
        </div>
        {#each deals as d (d.id)}
          <a
            href="/v2/pipeline/{d.id}"
      ...
      <section class="v2-card" style="overflow:hidden">
        ...
        {#each contacts as c (c.id)}
          ...
          >
            ...
            <!-- A link now. This was deliberately dead text while
                 `/v2/contacts/<uuid>` answered 404, which is the reason
        ...
        {:else}
          <p class="v2-sub" style="padding:14px 15px;font-size:12.5px">
            Nobody here yet. <a href="/v2/contacts/new?account={account.id}">Add the person</a> you actually
            talk to.
          </p>
        {/each}
      </section>
      ...
      <section class="v2-card" style="overflow:hidden">
        <div class="v2-card-head">
          ...
          <a href="/v2/tickets">View all</a>
        </div>
        {#each tickets as t (t.id)}
          <!-- A link again: tickets is wired, so a real id sent to
               `/v2/tickets/<uuid>` opens the ticket. -->
          <a
            href="/v2/tickets/{t.id}"
            style="display:flex;gap:12px;align-items:center;padding:11px 15px;border-bottom:1px solid var(--v2-line-soft);color:inherit;text-decoration:none"
          >
        ...
        {:else}
          <p class="v2-sub" style="padding:14px 15px;font-size:12.5px">
            No tickets. <a href="/v2/tickets/new?account={account.id}">Raise one</a> if something is wrong.
          </p>
        {/each}
      </section>
      ...
      <section class="v2-card" style="overflow:hidden">
        <div class="v2-card-head">
          ...
          <a href="/v2/invoices">View all</a>
        </div>
        {#each invoices as inv (inv.id)}
          <!-- A link now: invoices is wired, so a real id sent to
               `/v2/invoices/<uuid>` opens the invoice. -->
          <a
