...
<PageHeader title="Edit {account.name}" center>
  {#snippet crumb()}
    ...
    <a href="/v2/accounts/{account.id}">{account.name}</a>
  {/snippet}
  {#snippet sub()}
    {[
      account.industry,
  ...
  <form class="v2-form" method="POST" action="?/save" use:enhance={check} novalidate>
    {#if saved}
      <div class="v2-next" style="margin-bottom:18px" role="status">
        ...
        <a class="v2-btn" href="/v2/accounts/{account.id}">Back to the account</a>
      </div>
    {/if}
    ...
    <div class="actions">
      ...
      <a class="v2-btn" href="/v2/accounts/{account.id}">Cancel</a>
    </div>
  </form>
</div>
