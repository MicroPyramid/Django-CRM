<script>
    ...
    function getActivityStyle(action) {
        const styles = /** @type {{ [key: string]: { bg: string, text: string, icon: string } }} */ ({
            'CREATE': { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-600 dark:text-green-400', icon: 'plus' },
            'UPDATE': { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400', icon: 'pencil' },
            'DELETE': { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-600 dark:text-red-400', icon: 'trash' },
            'VIEW': { bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-600 dark:text-gray-400', icon: 'eye' },
            'COMMENT': { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-600 dark:text-purple-400', icon: 'message' },
            'ASSIGN': { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-600 dark:text-orange-400', icon: 'user-plus' }
        });
        return styles[action] || { bg: 'bg-gray-100 dark:bg-gray-700', text: 'text-gray-500 dark:text-gray-400', icon: 'activity' };
    }
    ...
    function getEntityUrl(entityType, entityId) {
        ...
        const basePath = routes[entityType] || '/app';
                    ...
                    <div class="space-y-4">
                        {#each recentData.activities as activity}
                            ...
                            <div class="flex items-start gap-3">
                                <div class="h-8 w-8 {style.bg} rounded-full flex items-center justify-center flex-shrink-0">
                                    {#if style.icon === 'plus'}
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm text-gray-900 dark:text-white">
                                        ...
                                        <span class="text-gray-500 dark:text-gray-400">{activity.entityType.toLowerCase()}</span>
