	...
	<div class="board-list">
		{#if data?.boards?.length}
			{#each data.boards as board}
				<a class="board-card" href="/tasks/board/{board.id}">
					<div class="board-title">{board.name}</div>
				</a>
			{/each}
		{:else}
			<p class="no-boards">No boards found.</p>
		{/if}
...
<style>
	...
	.board-card {
		...
		background: #f4f5f7;
		border-radius: 12px;
		box-shadow: 0 2px 12px 0 rgba(9, 30, 66, 0.13);
	...
	.board-card:hover {
		background: #e4e6ea;
		box-shadow: 0 6px 24px 0 rgba(9, 30, 66, 0.18);
	...
	.board-create:hover {
		border-color: #0079bf;
		box-shadow: 0 2px 12px 0 rgba(9, 30, 66, 0.1);
	}
	.board-create input {
		border: none;
	...
	.board-create input:focus {
		border-bottom: 1.5px solid #0079bf;
	}
	.board-create button {
		background: #0079bf;
		color: #fff;
		...
		box-shadow: 0 1px 4px 0 rgba(9, 30, 66, 0.1);
	}
