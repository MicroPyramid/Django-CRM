...
export async function load({ params, locals, cookies }) {
	...
	try {
		...
		const contact = {
			...
			// Address fields (from Django address or direct fields)
			street: contactData.address?.street || contactData.street || null,
			city: contactData.address?.city || contactData.city || null,
			state: contactData.address?.state || contactData.state || null,
			country: contactData.address?.country || contactData.country || null,
			postalCode: contactData.address?.postcode || contactData.postal_code || null,

			// Owner info
			owner:
				contactData.assigned_to && contactData.assigned_to.length > 0
					? {
							id: contactData.assigned_to[0].id,
							name:
								contactData.assigned_to[0].user_details?.email || contactData.assigned_to[0].email,
							email:
								contactData.assigned_to[0].user_details?.email || contactData.assigned_to[0].email
						}
		...
		const accountRelationships = (contactData.accounts || response.accounts_list || []).map(
			(account) => ({
				account: {
					id: account.id,
					name: account.name,
