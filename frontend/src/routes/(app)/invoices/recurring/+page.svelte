<script>
	...
	async function updateFilters(newFilters) {
		...
		['search', 'is_active', 'frequency', 'account'].forEach((key) =>
	...
	use:enhance={() => {
		return async ({ result }) => {
			if (result.type === 'success') {
				...
				toast.error(/** @type {string} */ (result.data?.error) || 'Failed to create recurring invoice');
			}
		};
	}}
>
	...
	use:enhance={() => {
		return async ({ result }) => {
			if (result.type === 'success') {
				...
				toast.error(/** @type {string} */ (result.data?.error) || 'Failed to update recurring invoice');
			}
		};
	}}
>
	...
	use:enhance={() => {
		return async ({ result }) => {
			if (result.type === 'success') {
				...
				toast.error(/** @type {string} */ (result.data?.error) || 'Failed to delete recurring invoice');
			}
		};
	}}
>
	...
	{#snippet activitySection()}
		{#if selectedRecurring}
			<div class="border-t pt-4">
				<h4 class="mb-2 text-sm font-medium text-muted-foreground">Statistics</h4>
				<div class="space-y-2 text-sm">
					...
					<div class="flex justify-between">
						...
						<span>{selectedRecurring.startDate ? formatDate(selectedRecurring.startDate) : '-'}</span
						>
					</div>
