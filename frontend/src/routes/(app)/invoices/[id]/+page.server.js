...
export const actions = {
	update: async ({ params, request, locals, cookies }) => {
		try {
			...
			const invoiceData = {
				...
				tax_rate: Math.max(0, Math.min(100, parseFloat(form.get('taxRate')?.toString() || '0') || 0)),
				discount_type: '',
				discount_value: '0',
				// Notes
	...
	markPaid: async ({ params, request, locals, cookies }) => {
		try {
			...
			const paymentDate = form.get('paymentDate')?.toString() || new Date().toISOString().split('T')[0];
