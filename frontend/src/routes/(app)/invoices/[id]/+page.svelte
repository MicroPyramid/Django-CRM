<script>
	...
	$effect(() => {
		if (data.invoice.id !== syncedInvoiceId) {
			...
			lineItems = data.invoice.lineItems?.length > 0
	...
	function addLineItem() {
		lineItems = [...lineItems, { id: crypto.randomUUID(), name: '', description: '', quantity: 1, rate: 0, amount: 0 }];
	}
...
>
	...
	<input type="hidden" name="lineItems" value={JSON.stringify(lineItems.filter(item => item.name || item.description || item.rate > 0))} />
</form>
...
<div class="min-h-screen bg-muted/30 print:bg-white">
	<!-- Header (hidden when printing) -->
	<div class="sticky top-0 z-10 border-b bg-background print:hidden">
				...
				<div>
					<div class="flex items-center gap-3">
						...
						<Select.Root type="single" value={invoice.status} onValueChange={handleStatusChange}>
							<Select.Trigger class="h-7 w-auto gap-1 border-0 px-2 text-xs font-medium {STATUS_COLORS[invoice.status] || STATUS_COLORS.Draft}">
								...
								{#each INVOICE_STATUS_OPTIONS as option}
									<Select.Item value={option.value}>
										<span class="flex items-center gap-2">
											<span class="size-2 rounded-full {STATUS_COLORS[option.value]?.split(' ')[0]}"></span>
											{option.label}
										</span>
									</Select.Item>
								{/each}
					...
					<p class="text-sm text-muted-foreground">{invoice.clientName || 'No client name'}</p>
				</div>
			</div>
			<div class="flex items-center gap-3">
				{#if isEditing}
				...
				{:else}
					...
					{#if invoice.status !== 'Cancelled' && invoice.status !== 'Paid'}
						<AlertDialog.Root bind:open={cancelDialogOpen}>
							<AlertDialog.Trigger>
								{#snippet child({ props })}
									<Button {...props} variant="outline" size="icon" class="text-destructive" title="Cancel Invoice">
										<Ban class="size-4" />
									</Button>
								{/snippet}
							</AlertDialog.Trigger>
							<AlertDialog.Content>
								<AlertDialog.Header>
									...
									<AlertDialog.Description>
										This will cancel invoice {invoice.invoiceNumber}. Cancelled invoices cannot be sent or edited.
									</AlertDialog.Description>
								</AlertDialog.Header>
								<AlertDialog.Footer>
	...
	<div class="mx-auto max-w-6xl px-6 py-8 print:max-w-none print:px-0 print:py-0">
		...
		{:else}
			<!-- Edit Mode -->
			<div class="rounded-lg border bg-background shadow-sm print:hidden">
			<!-- Invoice Header -->
					...
					<div class="space-y-4">
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								<Label class="w-24 text-muted-foreground">Invoice #</Label>
								<span class="font-medium">{invoice.invoiceNumber}</span>
							</div>
							{#if isEditing}
								<div class="flex items-center gap-2">
									<Label class="w-24 text-muted-foreground">Title</Label>
									<Input bind:value={invoice.invoiceTitle} placeholder="Invoice title" class="max-w-xs" />
								</div>
								<div class="flex items-center gap-2">
									<Label class="w-24 text-muted-foreground">PO Number</Label>
									<Input bind:value={invoice.poNumber} placeholder="Purchase Order #" class="max-w-xs" />
								</div>
								<div class="flex items-center gap-2">
									<Label class="w-24 text-muted-foreground">Billing Period</Label>
									<Input bind:value={invoice.billingPeriod} placeholder="e.g. December 2024" class="max-w-xs" />
								</div>
					...
					<div class="space-y-3 text-right">
						{#if isEditing}
							<div class="flex items-center justify-end gap-2">
								...
								<Select.Root type="single" value={invoice.currency} onValueChange={(v) => (invoice.currency = v)}>
			...
			<div class="grid grid-cols-2 gap-8 border-b p-6">
				...
				<div class="space-y-3">
					...
					{#if isEditing}
						<Select.Root type="single" value={invoice.accountId} onValueChange={(v) => (invoice.accountId = v)}>
					...
					{:else}
						<p class="font-medium">{accounts.find((a) => a.id === invoice.accountId)?.name || 'No account'}</p>
					{/if}
				</div>
					...
					{:else}
						<div class="space-y-1">
							...
							{#if invoice.clientEmail}
								<p class="text-sm text-muted-foreground">{invoice.clientEmail}</p>
							{/if}
							{#if invoice.clientPhone}
								<p class="text-sm text-muted-foreground">{invoice.clientPhone}</p>
							{/if}
						</div>
					{/if}
				</div>
			...
			<div class="p-6">
				...
				<div class="rounded-md border">
					<div class="grid grid-cols-12 gap-4 border-b bg-muted/50 px-4 py-3 text-sm font-medium text-muted-foreground">
								...
								{#if isEditing}
									<div class="space-y-2">
										...
										<Input
											...
											class="border-0 bg-transparent px-0 text-xs text-muted-foreground focus-visible:ring-0"
										/>
									</div>
								{:else}
									<div>
				...
				<div class="w-80 space-y-3">
					...
					<div class="flex items-center justify-between text-sm">
						<div class="flex items-center gap-2">
							...
							{#if isEditing}
								<Input type="number" bind:value={taxRate} min="0" max="100" step="0.1" class="h-7 w-16 text-right text-xs" />
					...
					{#if Number(invoice.amountPaid) > 0}
						...
						<div class="flex justify-between font-medium">
							...
							<span class="text-orange-600">{formatCurrency(Number(invoice.amountDue), invoice.currency)}</span>
						</div>
					{/if}
				</div>
			</div>
			...
			<div class="grid grid-cols-2 gap-8 border-t p-6">
				<div class="space-y-2">
					...
					<Textarea bind:value={invoice.notes} placeholder="Notes visible to client..." rows={4} />
				</div>
				<div class="space-y-2">
