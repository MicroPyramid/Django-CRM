<script>
	...
	const ESTIMATE_STATUSES = [
		...
		{ value: 'Viewed', label: 'Viewed', color: 'indigo' },
				...
				<div class="flex gap-1">
					{#each STATUS_CHIPS as chip}
						<button
							...
							class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium transition-colors {statusChipFilter === chip.key
	...
	>
		{#snippet emptyState()}
			<div class="flex flex-col items-center justify-center py-16 text-center">
				...
				<h3 class="text-foreground text-lg font-medium">No estimates yet</h3>
				<p class="text-muted-foreground text-sm">Create your first estimate to get started</p>
			</div>
		{/snippet}
		{#snippet cellContent(row, column)}
			...
			{:else if column.key === 'expiryDate'}
				{#if row.expiryDate}
					<span class={isExpired(row.expiryDate) && row.status !== 'Accepted' ? 'text-red-600' : ''}>
	...
	{#snippet activitySection()}
		{#if selectedEstimate}
			<div class="border-t pt-4">
				<h4 class="mb-2 text-sm font-medium text-muted-foreground">Activity</h4>
