<script>
  ...
  const timelineItems = $derived.by(() => {
    ...
    for (const c of comments) {
      items.push({
        id: `comment-${c.id}`,
        ts: c.commented_on || c.commented_on_arrow || '',
        kind: 'comment',
        payload: c
      });
    }
    ...
    if (opp?.created_at || opp?.created_on) {
      items.push({
        id: `created-${opp.id}`,
        ts: opp.created_at || opp.created_on,
        kind: 'created',
        payload: opp
      });
    }
...
<PageHeader
  ...
  breadcrumb={[{ label: 'Opportunities', href: '/opportunities' }, { label: opp?.name || '—' }]}
>
  {#snippet meta()}
    <div
  ...
  {#snippet amount()}
    {#if opp?.amount != null}
      <span class="text-[28px] leading-none font-bold text-[color:var(--text)] tabular-nums">
  ...
  {#snippet actions()}
    <div class="flex items-center gap-1.5">
      ...
      <Button
        variant="outline"
        size="sm"
        onclick={() => opp?.id && goto(`/opportunities/${opp.id}/edit`)}
      >
    ...
    <div class="grid grid-cols-1 gap-6 pt-4 pb-8 lg:grid-cols-[1fr_320px]">
      ...
      <div class="flex flex-col gap-6">
        ...
        <SectionCard title="About">
          {#if opp?.description}
            <p class="text-[13px] leading-[1.6] text-[color:var(--text-muted)]">
              {opp.description}
            </p>
          {:else}
      ...
      <div class="flex flex-col gap-6">
        <SectionCard title="Details">
          <dl class="grid grid-cols-1 gap-y-3 text-[12px]">
            <div class="flex items-baseline justify-between gap-3">
              ...
              <dd class="truncate text-right font-medium text-[color:var(--text)]">
                {opp?.account?.name || '—'}
              </dd>
            </div>
            <div class="flex items-baseline justify-between gap-3">
            ...
            <div class="flex items-baseline justify-between gap-3">
              ...
              <dd class="text-right text-[color:var(--text-muted)]">
                {opp?.closed_on ? formatDate(opp.closed_on) : '—'}
              </dd>
            </div>
            <div class="flex items-baseline justify-between gap-3">
              <dt class="text-[color:var(--text-subtle)]">Probability</dt>
              <dd class="text-right text-[color:var(--text-muted)] tabular-nums">
        ...
        <SectionCard title="Contacts">
          {#if contacts.length === 0}
            <p class="text-[12px] text-[color:var(--text-subtle)] italic">No contacts linked.</p>
          {:else}
            <ul class="flex flex-col gap-2">
              {#each contacts as ct (ct.id)}
                <li class="flex items-center gap-2 text-[12px]">
                  <span
                    class="flex size-6 items-center justify-center rounded-full bg-[color:var(--bg-elevated)] text-[10px] font-medium text-[color:var(--text-muted)]"
                  >
                    {getNameInitials(ct.first_name || ct.email || '?', ct.last_name || '')}
                  </span>
                  <span class="truncate text-[color:var(--text)]"
            ...
            {#each attachments as a (a.id)}
              <li class="flex items-center gap-3 py-2.5 text-[12px]">
                ...
                <span class="text-[11px] text-[color:var(--text-subtle)]"
                  >{a.created_on ? formatRelativeDate(a.created_on) : ''}</span
                >
              </li>
            {/each}
          </ul>
