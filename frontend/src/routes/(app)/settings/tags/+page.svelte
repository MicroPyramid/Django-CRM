	...
	<Card.Root>
		...
		<Card.Content>
			{#if activeTags.length === 0}
				<p class="text-muted-foreground text-sm">No tags created yet. Click "New Tag" to create your first tag.</p>
			{:else}
	...
	{#if archivedTags.length > 0}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Hidden from selection but preserved in existing associations</Card.Description>
			</Card.Header>
			<Card.Content>
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{#each archivedTags as tag (tag.id)}
						<div class="flex items-center justify-between rounded-lg border border-dashed p-3 opacity-60">
							<Badge class={getTagColorClass(tag.color)}>{tag.name}</Badge>
		...
		>
			<div class="grid gap-4 py-4">
				<div class="grid gap-2">
					...
					<Input id="name" name="name" bind:value={formName} placeholder="e.g., VIP, Hot Lead, Priority" />
				</div>
				<div class="grid gap-2">
					<Label>Color</Label>
					<div class="flex flex-wrap gap-2">
						{#each TAG_COLORS as color}
							<button
								...
								class="flex h-8 w-8 items-center justify-center rounded-full border-2 transition-all {color.class} {formColor === color.value ? 'ring-primary ring-2 ring-offset-2' : 'hover:scale-110'}"
								title={color.label}
							>
				...
				<div class="grid gap-2">
					...
					<Input id="description" name="description" bind:value={formDescription} placeholder="What is this tag used for?" />
				</div>
			</div>
			<Dialog.Footer>
