    ...
    >
      {#snippet title()}
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-[16px] leading-[1.3] font-medium text-[color:var(--text-primary)]">
              Import {activeJob.status === 'COMPLETED'
                ? 'Completed'
                : activeJob.status === 'CANCELLED'
                  ? 'Cancelled'
                  : 'Failed'}
      ...
      <div class="flex items-center justify-between text-sm">
        ...
        <div class="flex gap-4 text-xs">
          <span class="text-emerald-600 dark:text-emerald-400"
            >{activeJob.imported_count || 0} imported</span
          >
          <span class="text-amber-600 dark:text-amber-400"
            >{activeJob.skipped_count || 0} skipped</span
          >
    ...
    <SectionCard>
      {#snippet title()}
        <div class="flex min-w-0 flex-col gap-0.5">
          ...
          <p class="text-[12px] text-[color:var(--text-muted)]">
            Choose which Salesforce objects you want to import. Dependencies will be resolved
            automatically.
          </p>
        </div>
      {/snippet}
            ...
            <div class="flex-1">
              ...
              {#if objType.depends.length > 0}
                <div class="text-muted-foreground/70 mt-1 text-xs">
                  Depends on: {objType.depends.join(', ')}
                </div>
              {/if}
            </div>
          </button>
      ...
      <div class="mt-6 flex items-center justify-between">
        ...
        >
          ...
          <Button
            type="submit"
            disabled={selectedTypes.size === 0 || isStarting}
        ...
        <Table.Body>
          {#each importHistory as job}
            ...
            >
              ...
              <Table.Cell
                ><Badge class={getStatusBadgeClass(job.status)}>{job.status}</Badge></Table.Cell
              >
              <Table.Cell class="text-muted-foreground max-w-[200px] truncate text-sm">
                {(job.object_types || []).join(', ') || 'None'}
              </Table.Cell>
              <Table.Cell class="text-right text-sm text-emerald-600 dark:text-emerald-400"
                >{job.imported_count || 0}</Table.Cell
              >
              <Table.Cell class="text-right text-sm text-amber-600 dark:text-amber-400"
                >{job.skipped_count || 0}</Table.Cell
              >
              <Table.Cell class="text-right text-sm text-red-600 dark:text-red-400"
                >{job.error_count || 0}</Table.Cell
              >
            </Table.Row>

            {#if isExpanded}
              {@const errors = (job.error_log || []).filter(
                (/** @type {any} */ e) => e.level !== 'skip'
              )}
              {@const skips = (job.error_log || []).filter(
                (/** @type {any} */ e) => e.level === 'skip'
              )}
                ...
                <Table.Cell colspan={7} class="bg-muted/30 p-0">
                  <div class="space-y-4 p-4">
                    {#if errors.length > 0}
                      <div>
                        <h4
                          class="mb-2 flex items-center gap-1.5 text-xs font-semibold tracking-wide text-red-600 uppercase dark:text-red-400"
                        >
                        ...
                        <div class="max-h-60 space-y-1 overflow-y-auto">
                          {#each errors as err}
                            <div
                              class="border-border flex items-start gap-2 rounded border bg-white px-3 py-2 dark:bg-gray-900/50"
                            >
                              <XCircle class="mt-0.5 size-3.5 shrink-0 text-red-500" />
                              <div class="min-w-0 flex-1 text-xs">
                                <span class="text-muted-foreground"
                                  >{getObjectLabel(err.object_type)}</span
                                >
                                {#if err.salesforce_id}
                                  <span class="text-muted-foreground/60 ml-1"
                                    >({err.salesforce_id})</span
                                  >
                                {/if}
                    ...
                    {#if skips.length > 0}
                      <div>
                        <h4
                          class="mb-2 text-xs font-semibold tracking-wide text-amber-600 uppercase dark:text-amber-400"
                        >
                          {skips.length}
                          {skips.length === 1 ? 'Record' : 'Records'} Skipped
                        </h4>
                        <div class="max-h-40 space-y-1 overflow-y-auto">
                          {#each skips as skip}
                            ...
                            >
                              ...
                              <div class="min-w-0 flex-1 text-xs">
                                <span class="text-muted-foreground"
                                  >{getObjectLabel(skip.object_type)}</span
                                >
                                {#if skip.salesforce_id}
                                  <span class="text-muted-foreground/60 ml-1"
                                    >({skip.salesforce_id})</span
                                  >
                                {/if}
