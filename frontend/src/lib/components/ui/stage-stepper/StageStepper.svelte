  ...
  {#each stages as stage, i (stage.value)}
    ...
    <div
      ...
      class={cn(
        ...
        state === 'done' &&
          'border-[color:var(--border-faint)] bg-[color:var(--bg-elevated)] text-[color:var(--text-muted)]',
        state === 'future' &&
          'border-[color:var(--border-faint)] bg-[color:var(--bg-elevated)] text-[color:var(--text-subtle)]'
      )}
    >
      <span
        class={cn(
          'truncate text-[10px] leading-none font-semibold tracking-[0.04em] uppercase',
          state === 'current'
            ? ''
            : state === 'done'
