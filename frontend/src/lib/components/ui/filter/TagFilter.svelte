    ...
    <Popover.Trigger asChild class="">
      {#snippet child({ props })}
        ...
        >
          ...
          {#if hasSelection}
            <span
              class="inline-flex h-[18px] min-w-[18px] items-center justify-center rounded-[var(--r-sm)] px-1.5 text-[10.5px] font-medium tabular-nums text-[color:var(--violet-soft-text)] opacity-90"
            >
              {selectedIds.length}
            </span>
            <span
