<script>
	...
	const companyCityLine = $derived(formatCityStateZip(company?.city, company?.state, company?.postcode));
	const clientCityLine = $derived(formatCityStateZip(invoice.clientCity, invoice.clientState, invoice.clientPostcode));
</script>

<div class="invoice-view {className}" style="--primary-color: {primaryColor}; --secondary-color: {secondaryColor};">
	<div class="invoice-container">
		...
		<header class="header">
			<div class="company-details">
				...
				{/if}
				{#if company?.website}
					<p>Web: <a href={company.website} target="_blank" rel="noopener noreferrer">{company.website.replace(/^https?:\/\//, '')}</a></p>
				{/if}
			</div>
			...
			<table class="totals-table">
				<tbody>
					<tr>
						...
						<td class="totals-value">{formatCurrency(Number(invoice.subtotal || 0), invoice.currency)}</td>
					</tr>
					<tr>
					...
					{#if Number(invoice.discountAmount || 0) > 0}
						<tr>
							...
							<td class="totals-value">-{formatCurrency(Number(invoice.discountAmount), invoice.currency)}</td>
						</tr>
					{/if}
					<tr>
					...
					<tr class="grand-total">
						...
						<td class="totals-value">{formatCurrency(Number(invoice.totalAmount || 0), invoice.currency)}</td>
					</tr>
					{#if Number(invoice.amountPaid || 0) > 0}
						<tr>
							<td class="totals-label">Amount Paid</td>
							<td class="totals-value paid">-{formatCurrency(Number(invoice.amountPaid), invoice.currency)}</td>
						</tr>
						<tr class="amount-due">
							<td class="totals-label">Amount Due</td>
							<td class="totals-value">{formatCurrency(Number(invoice.amountDue || 0), invoice.currency)}</td>
						</tr>
					{/if}
				</tbody>
			</table>
