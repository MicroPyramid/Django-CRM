...
from common.models import Address, APISettings, Comment, Company, Document, User
...
class RegistrationForm(forms.ModelForm):
    sub_domain = forms.CharField(max_length=100)
    ...
    def clean_sub_domain(self):
        ...
        if Company.objects.filter(sub_domain__iexact=sub_domain):
            raise forms.ValidationError("Company subdomain already exists.")
        return sub_domain
...
class CompanyLoginForm(forms.ModelForm):
    ...
    # def clean_sub_domain(self):
    #     subdomain = self.cleaned_data.get("sub_domain")
    #     if Company.objects.get(sub_domain__iexact=subdomain):
    #         return subdomain
    #     raise forms.ValidationError("Company with this sub_domain doesnot exists")
