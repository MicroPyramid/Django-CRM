...
def digg_paginator(context):  # pragma: no cover
    ...
    if pages <= LEADING_PAGE_RANGE_DISPLAYED + NUM_PAGES_OUTSIDE_RANGE + 1:
        ...
        page_range = [
            n
            for n in range(pages - TRAILING_PAGE_RANGE_DISPLAYED + 1, pages + 1)
            if n > 0 and n <= pages
        ]
        ...
        page_range = [
            n
            for n in range(page - ADJACENT_PAGES, page + ADJACENT_PAGES + 1)
            if n > 0 and n <= pages
        ]
        pages_outside_leading_range = [
            n + pages for n in range(0, -NUM_PAGES_OUTSIDE_RANGE, -1)
...
register.inclusion_tag("contact_list_digg_paginator.html", takes_context=True)(
    digg_paginator
)
