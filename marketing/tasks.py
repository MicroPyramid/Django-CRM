    ...
    try:
        ...
        for each_contact in contacts:
            ...
            campaign_log = CampaignLog.objects.create(
                contact=each_contact, campaign=campaign
            )
            ...
            if not (each_contact.is_bounced or each_contact.is_unsubscribed):
                if (each_contact.email not in blocked_emails) and (
                    ...
                    names_dict = {
                        ...
                        if each_contact.company_name
                        else "",
                        "last_name": each_contact.last_name
                        if each_contact.last_name
                        else "",
                        ...
                        "email_id": each_contact.id,
                        "name": each_contact.name + " " + each_contact.last_name
                        if each_contact.last_name
                        else "",
                    ...
                    from_email = (
                        str(campaign.from_name) + "<" + str(campaign.from_email) + ">"
                    )
...
def list_all_bounces_unsubscribes():
    ...
    bounces = requests.get(
        "https://api.sendgrid.com/api/unsubscribes.get.json?api_user="
        + settings.EMAIL_HOST_USER
        + "&api_key="
        + settings.EMAIL_HOST_PASSWORD
    )
