...
class BlockedDomainAddSerailizer(serializers.ModelSerializer):
    ...
    def validate_domain(self, domain):
        ...
        if self.instance:
            ...
            if BlockedDomain.objects.filter(
                domain__iexact=domain, company=self.company).exists():               
                raise serializers.ValidationError("Domain already exists with this name")
        return domain
...
class BlockedEmailAddSerailizer(serializers.ModelSerializer):
    ...
    def validate_email(self, email):
        if self.instance:
            if BlockedEmail.objects.filter(
            ...
            if BlockedEmail.objects.filter(
                email=email, company=self.company).exists():               
