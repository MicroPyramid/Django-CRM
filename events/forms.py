...
class EventForm(forms.ModelForm):
    ...
    recurring_days = forms.MultipleChoiceField(required=False, choices=WEEKDAYS)
    ...
    def clean_name(self):
        ...
        if not self.obj_instance:
            if Event.objects.filter(name=name).exclude(id=self.instance.id).exists():
                raise forms.ValidationError("Event with this name already exists.")
    ...
    def clean_end_time(self):
        ...
        if self.cleaned_data.get("start_time") > end_time:
            raise forms.ValidationError("End Time cannot be less than Start Time")
        return end_time
