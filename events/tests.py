...
class EventObjectTest(object):
    def setUp(self):
        ...
        self.user = User.objects.create(
            first_name="johnEvent",
            username="johnDoeEvent",
            email="johnDoeEvent@example.com",
            role="ADMIN",
        ...
        self.comment = Comment.objects.create(
            comment="test comment", event=self.event, commented_by=self.user
        )
        self.attachment = Attachments.objects.create(
            attachment="image.png", event=self.event, created_by=self.user
        )


class EventListTestCase(EventObjectTest, TestCase):
    def test_events_list(self):
        self.client.login(email="johnDoeEvent@example.com", password="password")
        response = self.client.get(reverse("events:events_list"))
        self.assertEqual(response.status_code, 200)

        self.client.login(email="janeDoeEvent@example.com", password="password")
        response = self.client.get(reverse("events:events_list"))
        self.assertEqual(response.status_code, 200)
...
class EventDetailTestCase(EventObjectTest, TestCase):
    def test_events_detail(self):
        self.client.login(email="johnDoeEvent@example.com", password="password")
        response = self.client.get(reverse("events:detail_view", args=(self.event.id,)))
        self.assertEqual(response.status_code, 200)
        ...
        self.client.login(email="joeEvent@example.com", password="password")
        response = self.client.get(reverse("events:detail_view", args=(self.event.id,)))
...
class AddCommentTestCase(EventObjectTest, TestCase):
    def test_event_add_comment(self):
        ...
        response = self.client.post(reverse("events:add_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class UpdateCommentTestCase(EventObjectTest, TestCase):
    def test_event_update_comment(self):
        ...
        self.client.login(email="johnDoeEvent@example.com", password="password")
        data = {"commentid": self.comment.id, "event_id": self.event.id, "comment": ""}
        response = self.client.post(reverse("events:edit_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class DeleteCommentTestCase(EventObjectTest, TestCase):
    def test_event_delete_comment(self):
        ...
        self.client.login(email="janeDoeEvent@example.com", password="password")
        response = self.client.post(reverse("events:remove_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class AddAttachmentTestCase(EventObjectTest, TestCase):
    def test_event_add_attachment(self):
        ...
        self.client.login(email="johnDoeEvent@example.com", password="password")
        response = self.client.post(reverse("events:add_attachment"), data)
        self.assertEqual(response.status_code, 200)
        ...
        data = {"attachment": "", "event_id": self.event.id}
        self.client.login(email="johnDoeEvent@example.com", password="password")
        response = self.client.post(reverse("events:add_attachment"), data)
        self.assertEqual(response.status_code, 200)
...
class DeleteAttachmentTestCase(EventObjectTest, TestCase):
    def test_invoice_delete_attachment(self):
        ...
        self.client.login(email="janeDoeEvent@example.com", password="password")
        response = self.client.post(reverse("events:remove_attachment"), data)
        self.assertEqual(response.status_code, 200)
