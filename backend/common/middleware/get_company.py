...
class GetProfileAndOrg(object):
    ...
    def process_request(self, request):
        ...
        # But allow /api/auth/profile/ which needs the org header to get the user's profile
        auth_skip_paths = [
            '/api/auth/login/',
            '/api/auth/google/',
            '/api/auth/register/',
        ...
        try :
            ...
            if request.headers.get("Authorization"):
                ...
                # Use simplejwt's AccessToken for proper validation
                try:
                    access_token = AccessToken(token)
                    user_id = access_token['user_id']
                except Exception as e:
                    # Token invalid, but don't block - let DRF authentication handle it
                    print(f'Token validation in middleware failed: {e}')
                    return
