...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--status',
            action='store_true',
            help='Check RLS status for all tables'
        )
        parser.add_argument(
            '--test',
            action='store_true',
            help='Test that RLS is working correctly'
        )
        parser.add_argument(
            '--verify-user',
            action='store_true',
    ...
    def handle(self, *args, **options):
        if connection.vendor != 'postgresql':
            self.stderr.write(
                self.style.ERROR('RLS is only supported on PostgreSQL')
            )
            return
    ...
    def check_status(self):
        ...
        with connection.cursor() as cursor:
            ...
            if is_super:
                self.stdout.write(
                    self.style.WARNING(f'  Database user "{user}" is a SUPERUSER - RLS will be bypassed!')
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(f'  Database user "{user}" is not a superuser - RLS will be enforced')
                )
