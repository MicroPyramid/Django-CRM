...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING('Checking models with nullable org fields:\n'))
        ...
        for app_label, model_name in models_with_nullable_org:
            try:
                ...
                if null_count > 0:
                    issues_found.append((app_label, model_name, null_count, total_count))
                    self.stdout.write(
        ...
        for app_label, model_name in models_with_required_org:
            try:
                ...
                if verbose:
                    self.stdout.write(
                        self.style.WARNING(f"  ⚠️  {app_label}.{model_name}: Model not found")
                    )
        ...
        if issues_found:
            self.stdout.write(
                self.style.ERROR(f"Found {len(issues_found)} models with NULL org values:\n")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS('All models have proper org values! Ready for NOT NULL migration.')
            )
    ...
    def _fix_null_orgs(self, issues):
        ...
        for app_label, model_name, null_count, total_count in issues:
            ...
            if model_name == 'Address':
                ...
                self.stdout.write(
                    f"  Fixed {fixed}/{null_count} Address records"
                )
                if fixed < null_count:
                    self.stdout.write(
                        self.style.WARNING(
