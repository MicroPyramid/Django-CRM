...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--db-host', type=str, required=True, help='Prisma database host')
        parser.add_argument('--db-port', type=int, default=5432, help='Prisma database port')
        parser.add_argument('--db-name', type=str, required=True, help='Prisma database name')
        parser.add_argument('--db-user', type=str, required=True, help='Prisma database user')
        parser.add_argument('--db-password', type=str, required=True, help='Prisma database password')
        ...
        parser.add_argument('--only', type=str, help='Comma-separated list of models to migrate')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'Failed to connect to Prisma database: {e}'))
            return
            ...
            if not self.dry_run:
                with connection.cursor() as cursor:
                    ...
                    ''', [
                        ...
                        row['state'],
                        row['postalCode'],
                        row['country'][:2] if row['country'] else None,  # Convert to 2-letter code
                        row['annualRevenue'],
                        row['description'],
                ...
                owner_profile_id = self.user_to_profile.get(row['ownerId'], {}).get(row['organizationId'])
                if owner_profile_id:
                    with connection.cursor() as cursor:
                        cursor.execute('''
    ...
    def migrate_account_contacts(self):
        ...
        self.stdout.write(self.style.SUCCESS(f'  Migrated {migrated} account-contact relationships'))
    ...
    def migrate_leads(self):
        ...
        for row in rows:
            ...
            source = LEAD_SOURCE_MAP.get(row['leadSource'], 'other') if row['leadSource'] else None
    ...
    def migrate_cases(self):
        ...
        for row in rows:
            ...
            # Map priority
            priority = row['priority'] if row['priority'] in ['Low', 'Normal', 'High', 'Urgent'] else 'Normal'
    ...
    def migrate_comments(self):
        ...
        for row in rows:
            ...
            author_profile_id = self.user_to_profile.get(row['authorId'], {}).get(row['organizationId'])
            ...
            if not self.dry_run:
                with connection.cursor() as cursor:
                    ...
                    ''', [
                        ...
                        content_type.id,
                        object_id,
                        row['body'][:255] if row['body'] else '',  # Truncate to 255 chars
                        row['createdAt'],
                        author_profile_id,
                        row['organizationId'],
                        row['createdAt'],
    ...
    def migrate_board_members(self):
        ...
        for row in rows:
            ...
            profile_id = self.user_to_profile.get(row['userId'], {}).get(row['organizationId'])
        ...
        for row in rows:
            ...
            if not self.dry_run:
                ...
                if row['assigneeId'] and org_id:
                    profile_id = self.user_to_profile.get(row['assigneeId'], {}).get(org_id)
                    if profile_id:
                        with connection.cursor() as cursor:
                            cursor.execute('''
    ...
    def migrate_contact_submissions(self):
        ...
        for row in rows:
            if self.skip_existing and ContactFormSubmission.objects.filter(id=row['id']).exists():
                skipped += 1
                continue
        ...
        self.stdout.write(self.style.SUCCESS(f'  Migrated {migrated} contact submissions'))
