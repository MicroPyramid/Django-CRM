...
from common.models import Attachments, Comment, User, Company
...
class TaskCreateTest(object):
    def setUp(self):
        ...
        self.user = User.objects.create(
            first_name="johnTask",
            username="johnDoeTask",
            email="johnTask@example.com",
            role="ADMIN",
        ...
        self.comment = Comment.objects.create(
            comment="test comment", task=self.task, commented_by=self.user
        )
        self.attachment = Attachments.objects.create(
            attachment="image.png", task=self.task, created_by=self.user
        )
...
class TaskListTestCase(TaskCreateTest, TestCase):
    def test_tasks_list(self):
        ...
        data = {"task_title": "title", "status": "New", "priority": "Medium"}
...
class TaskCreateTestCase(TaskCreateTest, TestCase):
    def test_task_create(self):
        ...
        self.client.login(email="johnTask@example.com", password="password")
        data = {"title": "", "status": "New", "priority": "Medium"}
...
class TaskDetailTestCase(TaskCreateTest, TestCase):
    def test_task_detail(self):
        ...
        response = self.client.get(reverse("tasks:task_detail", args=(self.task_1.id,)))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse("tasks:task_detail", args=(self.task_2.id,)))
        self.assertEqual(response.status_code, 200)
...
class TaskEditTestCase(TaskCreateTest, TestCase):
    def test_task_Edit(self):
        self.client.login(email="johnTask@example.com", password="password")
        response = self.client.get(reverse("tasks:task_edit", args=(self.task.id,)))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse("tasks:task_edit", args=(self.task_2.id,)))
        self.assertEqual(response.status_code, 200)
...
class TaskDeleteTestCase(TaskCreateTest, TestCase):
    def test_task_delete(self):
        self.client.login(email="janeDoeTask@example.com", password="password")
        response = self.client.get(reverse("tasks:task_delete", args=(self.task.id,)))
        self.assertEqual(response.status_code, 403)
        ...
        response = self.client.get(
            reverse("tasks:task_delete", args=(self.task_1.id,))
...
class AddCommentTestCase(TaskCreateTest, TestCase):
    def test_task_add_comment(self):
        ...
        response = self.client.post(reverse("tasks:add_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class UpdateCommentTestCase(TaskCreateTest, TestCase):
    def test_task_update_comment(self):
        ...
        self.client.login(email="johnTask@example.com", password="password")
        data = {"commentid": self.comment.id, "task_id": self.task.id, "comment": ""}
        response = self.client.post(reverse("tasks:edit_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class DeleteCommentTestCase(TaskCreateTest, TestCase):
    def test_task_delete_comment(self):
        ...
        self.client.login(email="janeDoeTask@example.com", password="password")
        response = self.client.post(reverse("tasks:remove_comment"), data)
        self.assertEqual(response.status_code, 200)
...
class AddAttachmentTestCase(TaskCreateTest, TestCase):
    def test_task_add_attachment(self):
        ...
        self.client.login(email="johnTask@example.com", password="password")
        response = self.client.post(reverse("tasks:add_attachment"), data)
        self.assertEqual(response.status_code, 200)
        ...
        data = {"attachment": "", "task_id": self.task.id}
        self.client.login(email="janeDoeTask@example.com", password="password")
        response = self.client.post(reverse("tasks:add_attachment"), data)
        self.assertEqual(response.status_code, 200)
...
class DeleteAttachmentTestCase(TaskCreateTest, TestCase):
    def test_task_delete_attachment(self):
        ...
        self.client.login(email="janeDoeTask@example.com", password="password")
        response = self.client.post(reverse("tasks:remove_attachment"), data)
        self.assertEqual(response.status_code, 200)
