...
class CaseCreation(object):
    def setUp(self):
        self.address = Address.objects.create(
            street="6th phase", city="hyderabad", postcode="506344", country="IN",
        )

        self.user = User.objects.create(
            first_name="raghu", username="raghu", email="r@mp.com", role="ADMIN",
        )
...
class CaseRemoveTestCase(CaseCreation, TestCase):
    ...
    def test_case_delete(self):
        response = self.client.post(
            "/cases/" + str(self.case.id) +
            "/remove/", {"case_id": self.case.id},
        )
        self.assertEqual(response.status_code, 200)
...
class CaseUpdateTestCase(CaseCreation, TestCase):
    ...
    def test_case_update(self):
        response = self.client.post(
            "/cases/" + str(self.case.id) +
            "/edit_case/", {"hiddenval": self.case.id},
        )
        self.assertEqual(response.status_code, 200)
