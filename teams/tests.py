...
class TeamListTestCase(TeamObjectTest, TestCase):
    def test_teams_list(self):
        self.client.login(email="johnDoeTeam@example.com", password="password")
        response = self.client.get(reverse("teams:teams_list"))
        self.assertEqual(response.status_code, 200)

        self.client.login(email="janeDoeTeam@example.com", password="password")
        response = self.client.get(reverse("teams:teams_list"))
...
class TeamCreateCase(TeamObjectTest, TestCase):
    def test_team_create(self):
        ...
        data = {"name": "", "users": self.user1.id, "description": "team description"}
...
class TeamEditCase(TeamObjectTest, TestCase):
    def test_team_edit(self):
        self.client.login(email="johnDoeTeam@example.com", password="password")
        response = self.client.get(reverse("teams:team_edit", args=(self.team_dev.id,)))
        self.assertEqual(response.status_code, 200)
