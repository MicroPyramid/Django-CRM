...
class OpportunityListView(APIView, LimitOffsetPagination):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["users"] = ProfileSerializer(
            Profile.objects.filter(is_active=True, org=self.request.org).order_by("user__email"),
            many=True,
        ).data
        tag_ids = list(set(Opportunity.objects.filter(org=self.request.org).values_list("tags", flat=True)))
        ...
        if self.request.profile == "ADMIN":
            context["teams_list"] = TeamsSerializer(Teams.objects.filter(org=self.request.org), many=True).data
    ...
    def put(self, request, pk, format=None):
        ...
        if serializer.is_valid():
            ...
            if params.get("stage"):
                stage = params.get("stage")
                if stage in ["CLOSED WON", "CLOSED LOST"]:
                    opportunity_object.closed_by_id = self.request.profile.id
    ...
    def get(self, request, pk, format=None):
        ...
        if self.request.user.is_superuser or self.request.profile.role == "ADMIN":
            ...
            if self.opportunity.created_by:
                users_mention = [{"username": self.opportunity.created_by.user.username}]
            else:
                users_mention = []
        else:
            users_mention = []
