...
class OpportunityModel(object):
    def setUp(self):
        ...
        self.user = User.objects.create(
            first_name="jane doe",
            username="jane doe Opp",
            email="janeOpp@example.com",
            role="ADMIN",
        ...
        self.address = Address.objects.create(
            street="", city="city name", postcode="1234", country="IN"
        )
...
class CommentTestCase(OpportunityModel, TestCase):
    ...
    def test_comment_create(self):
        response = self.client.post(
            ...
            {"opportunityid": self.opportunity.id, "comment": "comment"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_form_valid(self):
        response = self.client.post(
            "/opportunities/comment/add/",
            {"opportunityid": self.opportunity.id, "comment": "hello"},
        )
        self.assertEqual(response.status_code, 200)
...
class TestOpportunityListViewForUser(OpportunityModel, TestCase):
    def test_queryset_for_user(self):
        ...
        self.assertJSONEqual(
            force_text(response.content), {"error": ["This field is required."]}
        )
    ...
    def test_detail_view_error(self):
        ...
        response = self.client.post(
            reverse("opportunity:opp_remove", args=(self.opportunity.id,)),
...
class AttachmentTestCaseError(OpportunityModel, TestCase):
    def test_attachment_add(self):
        ...
        response = self.client.get(
            reverse("opportunity:save") + "?view_account={}".format(self.account.id)
        )
        self.assertEqual(200, response.status_code)

        response = self.client.get(
            reverse("opportunity:opp_view", args=(self.opportunity_1.id,))
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.get(
            reverse("opportunity:opp_view", args=(self.opportunity_2.id,))
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.post(
            reverse("opportunity:opp_edit", args=(self.opportunity_2.id,)), data
        )
        self.assertEqual(200, response.status_code)

        response = self.client.get(
            reverse("opportunity:opp_edit", args=(self.opportunity_2.id,))
        ...
        self.client.login(email="joeDoeOpp@example.com", password="password")
        response = self.client.get(
            reverse("opportunity:opp_edit", args=(self.opportunity_1.id,)), data
        )
        self.assertEqual(403, response.status_code)
        ...
        self.client.login(email="janeOpp@example.com", password="password")
        response = self.client.post(
            reverse("opportunity:opp_remove", args=(self.opportunity_2.id,))
            + "?view_account={}".format(self.account.id),
        ...
        response = self.client.get(
            reverse("opportunity:contacts") + "?account={}".format(self.account.id)
        )
        self.assertEqual(200, response.status_code)
