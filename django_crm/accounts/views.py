...
from .models import LeadAccount, INDCHOICES, TYPECHOICES, TEAMS, Comment
from ..djcrm.models import Country, Address
from django_crm.oppurtunity.models import Opportunity, STAGES, SOURCES
from ..contacts.models import Contact
from ..cases.models import Case
from .forms import AccountForm, BillingAddressForm, ShippingAddressForm, CommentForm
...
def account(request):
    ...
    accounts_list = LeadAccount.objects.all()
    page = request.POST.get('per_page')
    if request.GET.get('name'):
        accounts_list = LeadAccount.objects.filter(
...
def remove_account(request, aid):
    ...
    LeadAccount.objects.filter(id=aid).delete()
    # count_list = LeadAccount.objects.all().count()
...
def view_account(request, aid):
    acc = LeadAccount.objects.get(id=aid)
...
def edit_account(request, edid):
    account_instance = get_object_or_404(LeadAccount, id=edid)
...
def comment_add(request):
    if request.method == 'POST':
        account = get_object_or_404(LeadAccount, id=request.POST.get('accountid'))
        assigned = False
        ...
        if request.user == account.users or assigned:
            ...
            if form.is_valid():
                ...
                c = Comment.objects.create(comment=comment, accountid=LeadAccount.objects.get(id=accountid), comment_user=comment_user)
...
def get_accounts(request):
    if request.method == 'GET':
        if request.user.is_authenticated():
            accounts = LeadAccount.objects.all()
