...
from .models import Lead, Comments, TEAMS, lead_status, lead_source
from ..contacts.forms import ContactForm
from django.views.decorators.csrf import csrf_exempt
from ..djcrm.models import Country, Address
from .forms import CommentForm, CreateLeadform, AddressLeadForm
from django.core.urlresolvers import reverse
from ..accounts.forms import AccountForm
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
from ..accounts.forms import BillingAddressForm, ShippingAddressForm
from ..accounts.models import LeadAccount, INDCHOICES, TYPECHOICES


@login_required
def add_lead(request):
    acount = LeadAccount.objects.all()
...
def display(request):
    ...
    if request.method == 'POST':
        ...
        if first_name:
            crm_task = Lead.objects.filter(first_name=first_name)

        if last_name:
            crm_task = Lead.objects.filter(last_name=last_name)

        if city:
            crm_task = Lead.objects.filter(address=Address.objects.filter
                                           (city__contains=city))
        ...
        return render(request, 'leads/leads.html',
                      {'crm_task': crm_task, 'dis_address_obj':
                       dis_address_obj, 'count': count})
    else:
        return render(request, 'leads/leads.html', {
