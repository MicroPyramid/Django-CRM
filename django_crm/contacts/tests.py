...
from ..djcrm.models import Address, Country
...
class ContactObjectsCreation(object):
    def setUp(self):
        ...
        self.account = LeadAccount.objects.create(name="account", email="account@gmail.com", phone="12345", billing_address=Address.objects.get(pk=1), shipping_address=Address.objects.get(pk=1), website="account.com", account_type="account", sis_code="12345", industry="IT", description="account")
        self.contact = Contact.objects.create(name="contact", email="contact@gmail.com", phone="12345", account=LeadAccount.objects.get(pk=1), address=Address.objects.get(pk=1), description="contact" )
