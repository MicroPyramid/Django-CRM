...
from .forms import ReminderForm, MeetingEventForm, CallEventForm, TaskEventForm
...
def add_parent(parent_type, parent_id, form, request_user):
    if parent_type == 'Account':
        account = LeadAccount.objects.get(id=int(parent_id))
...
def get_parent(parent_type, parent_id):
    if parent_type == 'Account':
        try:
            account = LeadAccount.objects.get(id=parent_id)
            return account
